...
variables = {
	...
	"IMAGE-IDS": {
		"value": "",
		"required": "false",
        "description":"Path of the Json file containing imageDigest and imageTag. Can get it from the output of aws_ecr_list_images."
	},
	"TAG-STATUS": {
		"value": "ANY",
		"required": "false",
...
def exploit(profile, workspace):
	...
	if tag_status == 'TAGGED' or tag_status == 'UNTAGGED' or tag_status == 'ANY':
		...
		file = "{}_ecr_describe_images".format(dt_string)
		filename = "./workspaces/{}/{}".format(workspace, file)

		repo_name = variables['REPO-NAME']['value']
		reg_id = variables['REGISTRY-ID']['value']
		image_ids = variables['IMAGE-IDS']['value']
		...
		if not image_ids == '':
			with open(image_ids, 'r') as json_file:
				image_ids_json = json.load(json_file)
			kwargs['imageIds'] = image_ids_json

		response = profile.describe_images(
			**kwargs
		)
		json_data = response['imageDetails']
		...
		while 'nextToken' in response:
			...
			json_data.extend(response['imageDetails'])
